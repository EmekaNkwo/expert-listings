...
export const Icon: React.FC<IconProps> = ({
  ...
  // Handle size - if it's a number, use it as pixels, otherwise use as className
  const sizeStyle = typeof size === 'number' ? { width: `${size}px`, height: `${size}px` } : {};
  const sizeClass = typeof size === 'string' ? size : '';
  
  // Default size classes for common sizes
  const defaultSizeClass = typeof size === 'number' 
    ? size === 16 ? 'h-4 w-4' 
    : size === 20 ? 'h-5 w-5'
    : size === 24 ? 'h-6 w-6'
    : ''
    ...
    >
      {React.Children.map(children, (child) => {
        if (React.isValidElement(child) && child.type === 'path') {
          const childProps = (child as React.ReactElement<React.SVGProps<SVGPathElement>>).props;
